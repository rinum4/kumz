%function [data_out]=model7p(id_t,id_in,r_in,d_in,p_in,su1_in,su2_in,db_data)
...
%S_NEH_f = KumzFullSp(NEH3,ma); % (perm) расписание - послед-й проход заказов:
...
%S_new_par = [];
%ri=1;
%в теории надо бы распараллелить
%while ri<=2*n
%ast1=tic;
for  ri = 1:2*n   %этот цикл не параллелится!
    ...
    for l=1:numel(S)+1 %parfor бесмысленнен
        if l==1
            S_new_par = [sr S];
        elseif l== numel(S) +1
            S_new_par = [S sr];
    ...
    if ri>=3
       ...
       parfor ii=1:numel(S)-1 %эффективен
          T_S_new(ii,:) = S_xchange(ii,S,ri,sr2, p1, n, m1, ma1, d1, su1, r);
        end
...
for  ri = 1:2*n   %этот цикл не параллелится!
    ...
    %C_min = inf;
    T_S_new=ones(1,numel(S))*inf;
    for l=1:numel(S) %parfor бесмысленнен
        if l==1
            S_new_par = [sr S];
        elseif l== numel(S) 
            S_new_par = [S sr];
...
S_1212f1 = KumzFullS12(S121_1); % (perm) расписание - послед-й проход заказов:
...
%пробуем простое решение
S_1212f2 = KumzFullS12(S121_2); % (perm) расписание - послед-й проход заказов:
...
%stt3 = tic;
%параллельная версия
[~,S_opt_SACO,S1_s,S2_e,S2_U]=SACO_KUMZ_12_x_12_4p(p, n, m, ma, d, su,S121_1,S121_2, r);  %
%endt3 = toc(stt3);
...
FF = [Fmaxp FNEH_f FT_1212_ab FT_1212f1 FT_1212f2 FT_12_SACO_12 FT_ab_1212 FT_joh_1212 FT_1212];
[~,ind] = min(FF);
SS = [S_maxp; S_NEH_f; S_1212_ab; S_1212f1; S_1212f2; S_12_SACO_12; S_ab_1212; S_joh_1212 S_1212];
Smin = SS(ind,:);
