%function  [db_data,id_t,id_in,r_in,d_in,p_in,su1_in,su2_in] = test_data_prep4(db_data)
...
if flag1==1
    ...
    db_data1 = table2cell(db_data(:,1:30));
...
for i=1:n %цикл по заказам
    ...
    for j=1:numel(id_n) %цикл по строкам
        ...
        %if id_in(jj)==f_srch % i
            if ~isnan(db_data{jj,12})
                date_r = date_b + db_data{jj,12}/3600/24;
...
for i=1:n %цикл по заказам
    ...
    for j=1:numel(id_n) %цикл по строкам
        jj= id_n(j);
        %if id_in(jj)==f_srch % i
            if ~isnan(db_data{jj,22})
                date_r = date_b + db_data{jj,22}/3600/24;
...
for i=1:n %цикл по заказам
    ...
    for j=1:numel(id_n) %цикл по строкам
        %if id_n(j)==f_srch % i
            ...
            %if db_data{j,4}{1}=='K' || db_data{j,4}{1}=='T'
            %    continue;
            %end
            jj=id_n(j);
            
            if ~isnan(db_data{jj,20})
                durat = db_data{jj,16}/60*db_data{jj,20};
            else
                durat = db_data{jj,16}/60;
            end    
            
            if strcmp(db_data{jj,3}{1},'S01::ST')
                %p(1,i)=p(1,i) + db_data{j,16}/60;
                continue;
            end % S01::ST
            
            if strcmp(db_data{jj,3}{1},'CRM::RS') ||  strcmp(db_data{jj,3}{1},'CRM::TX') %1рц
                ...
                if strcmp(db_data{jj,3}{1},'CRM::TX')
            ...
            if strcmp(db_data{jj,3}{1},'CTI::ST')  ||  strcmp(db_data{jj,3}{1},'CTI::TX') %473 ячейки
                ...
                if stage~=2 && stage~=8 && stage~=14
                    if db_data{jj,4}{1}~='K' && db_data{jj,4}{1}~='T'
                        stage = stage + 1;
                    end
                end
                continue;
            ...
            if strcmp(db_data{jj,3}{1},'EBC::FX')  ||  strcmp(db_data{jj,3}{1},'EBC::TX') %18 рц
                if stage == 0 || stage == 6 || stage == 12
                    %if db_data{j,4}{1}~='K' && db_data{j,4}{1}~='T'
                        stage = stage + 3;
                    %end
            ...
            if strcmp(db_data{jj,3}{1},'G03::CU')  ||  strcmp(db_data{jj,3}{1},'G03::IN')  ||  strcmp(db_data{jj,3}{1},'G03::PA')  ||  strcmp(db_data{jj,3}{1},'G03::PK')  %1рц
                if stage<5
                    stage=5;
                end    
                switch stage
            ...
            %2 рц
            if strcmp(db_data{jj,3}{1},'G01::CU')  ||  strcmp(db_data{jj,3}{1},'G01::IN')  ||  strcmp(db_data{jj,3}{1},'G01::PA')  ||  strcmp(db_data{jj,3}{1},'G01::PK') || ...
                    strcmp(db_data{jj,3}{1},'G02::CU')  ||  strcmp(db_data{jj,3}{1},'G02::IN')  ||  strcmp(db_data{jj,3}{1},'G02::PA')  ||  strcmp(db_data{jj,3}{1},'G02::PK')
                if stage<19
                    stage=19;
                end    
...
for i=1:n %цикл по заказам
    ...
    for l=1:numel(id_n) %цикл по строкам
        ...
        %if id_in(ll)==f_srch % i
            if strcmp(db_data{ll,3}{1},'CRM::RS')
                search_val = db_data{ll,28}{1};
                break;
            end
        %end
    end %l
    if ~strcmp(search_val, 'NaN')
        for jj=i+1:n %цикл по заказам
            ...
            for l=1:numel(id_n) %цикл по строкам
                ...
                if strcmp(db_data{ll,3}{1},'CRM::RS')
                    if ~strcmp(db_data{ll,28}{1},search_val)
                        su1_in(i,jj)=60;
                        su1_in(jj,i)=60;
                    end
                    break;
...
for i=1:n %цикл по заказам
    ...
    for l=1:numel(id_n) %цикл по строкам
        ll=id_n(l);
        %if id_in(ll)==f_srch % i
            if strcmp(db_data{ll,3}{1},'EBC::FX')
                k=k+1;
            end
            if strcmp(db_data{ll,3}{1},'G03::CU')
                if k==0
                    k=k+1;
                end
                search_val{k} = db_data{ll,28}{1};
            end
        %end
    end %l
    ...
    for jj=i+1:n %цикл по заказам
        ...
        for l=1:numel(id_n) %цикл по строкам
            ...
            %if id_in(ll)==f_srch % j
                ...
                if strcmp(db_data{ll,3}{1},'G03::CU')
                    ...
                    if ~strcmp(db_data{ll,28}{1},search_val{k}) && ~strcmp(search_val{k},'')
                        su2_in{k}(i,jj)=10;
                        su2_in{k}(jj,i)=10;
                    end
                end
        ...
        if k==2
            if tmp_l>0
                if ~strcmp(db_data{tmp_l,28}{1},search_val{3}) && ~strcmp(search_val{3},'')
                    su2_in{k}(i,jj)=10;
                    su2_in{k}(jj,i)=10;
                end
            end
