...
%S = KumzFullS(I,ma); % (perm) расписание - послед-й проход заказов:
...
%S = KumzFullS(NEH3,ma); % (perm) расписание - послед-й проход заказов:
...
%S = JeongFullS(S_NEH);
S = JeongFullSp(S_NEH);
%[T_NEH_1212,~,~,~]= XieTargFun7(p1, S, n, m1, ma1, d1, su1); %версия без r, т.к. вторая часть расписания от r не зависит
...
while ri<=2*n
    ...
    for l=1:numel(S) +1
        ...
        % важно! добавил r - для второй задачи r1 нулевой
...
if record>0 && flag_branch ==1 %22222222222
    ...
    %параллельный вариант
    [record,xmin] = JeongBranchAndBound11p(in_arg); %важно! в данной функции сменилась модель хранения su
